...
from celestial_resonances import resonances, critical, resonances_labels
...
def evo(infile, selected_sat):
	...
	total_records = 10 # TODO Remove me к хуям
	sat = Sat()
	...
	moon_res = np.ndarray(shape=(total_records, 12), dtype='float64')
	moon_crit = np.ndarray(shape=(total_records, 12), dtype='float64')
	sun_res = np.ndarray(shape=(total_records, 12), dtype='float64')
	sun_crit = np.ndarray(shape=(total_records, 12), dtype='float64')
	...
	a, i = evolution[0,2], evolution[0,4]/rad
	sat_dir = 'EVO/%d-%d/'% (a,i)
	...
	for i in range(1, sun_res.shape[1]):
		...
		plt.plot([sun_res[0,0], sun_res[-1, 0]], [0,0])
		plt.ylabel('Рез. соотн., рад/с')
		plt.xlabel('Время, годы')
